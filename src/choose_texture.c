...
/*   Updated: 2024/11/25 03:03:15 by aloiki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (game->map->map_aux[j - 1][i] == '0'
		&& game->map->map_aux[j + 1][i] == '0'
		&& game->map->map_aux[j][i - 1] == '0'
		&& game->map->map_aux[j][i + 1] == '0')
		return (game->textures->floor_all_open);
	if (game->map->map_aux[j - 1][i] == '1'
		&& game->map->map_aux[j + 1][i] == '1'
		&& game->map->map_aux[j][i - 1] == '0'
		&& game->map->map_aux[j][i + 1] == '0')
		return (game->textures->floor_top_bot);
	if (game->map->map_aux[j - 1][i] == '0'
		&& game->map->map_aux[j + 1][i] == '0'
		&& game->map->map_aux[j][i - 1] == '1'
		&& game->map->map_aux[j][i + 1] == '1')
