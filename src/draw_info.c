...
/*   Updated: 2024/11/25 03:06:11 by aloiki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_info(t_game *game, int moves)
{
	...
	mlx_string_put(game->window->mlx,
		game->window->win, 10, 10, 0xFFFFFF, str_to_print);
	free(str_to_print);
	str_to_print = ft_strjoin("Collectibles collectes: ",
			collectibles_collected);
	mlx_string_put(game->window->mlx,
		game->window->win, 10, 20, 0xFFFFFF, str_to_print);
	free(str_to_print);
	str_to_print = ft_strjoin("Collectibles remaining: ", collectibles_left);
	mlx_string_put(game->window->mlx,
		game->window->win, 10, 30, 0xFFFFFF, str_to_print);
	free(str_to_print);
	free(movements);
